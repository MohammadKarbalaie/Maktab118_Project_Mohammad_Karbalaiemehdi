...
const schema = z.object({
  ...
  password: z.string().min(8, 'رمز عبور باید حداقل 8 کاراکتر باشد').nonempty('رمز عبور الزامی است'),
});
...
const Login = () => {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      ...
      setErrorMessage(err.response?.data?.message || 'خطایی رخ داده است');
    }
  };
    ...
    <div className="w-screen h-screen overflow-hidden flex justify-center items-center bg-gray-50">
      ...
      >
        ...
        <div className="flex my-8 mx-4 md:mx-2 border-b-2 border-gray-700 hover:border-green-800">
          <label className="self-center w-24 ">نام کاربری</label>
          <input
            {...register('username')}
        ...
        <div className="flex items-center my-8 mx-4 md:mx-2 border-b-2 border-gray-700 hover:border-green-800">
          <label className="self-center w-16">رمز عبور</label>
        ...
        {errorMessage && (
          <p className="text-red-500 text-sm mx-4 md:mx-2">{errorMessage}</p>
        )}
